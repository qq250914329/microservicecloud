...
import com.fei.springboot.dbconfig.DataSourceContextHolder;
import com.fei.springboot.dbconfig.DataSourceType;
...
/*	@Before("execution(* com.fei.springboot.service..*.find*(..)) "
	...
	@Before("execution(* com.fei.springboot.service..*.find*(..)) or execution(* com.fei.springboot.service..*.get*(..)) or execution(* com.fei.springboot.service..*.query*(..))"
			+ " and @annotation(com.fei.springboot.annotation.ReadDataSource) ")
	public void setReadDataSourceType() {
		//如果已经开启写事务了，那之后的所有读都从写库读
		if(!DataSourceType.write.getType().equals(DataSourceContextHolder.getReadOrWrite())){
	...
	@Before("execution(* com.fei.springboot.service..*.insert*(..)) or execution(* com.fei.springboot.service..*.update*(..)) or execution(* com.fei.springboot.service..*.add*(..))"
