...
import com.fei.springboot.memory9991.dbconfig.DataSourceType;
...
/*	@Before("execution(* com.fei.springboot.service..*.find*(..)) "
	...
	@Before("execution(* com.fei.springboot.memory9991.service..*.find*(..)) or execution(* com.fei.springboot.memory9991.service..*.get*(..)) or execution(* com.fei.springboot.memory9991.service..*.query*(..))"
			+ " and @annotation(com.fei.springboot.annotation.ReadDataSource) ")
	public void setReadDataSourceType() {
		//如果已经开启写事务了，那之后的所有读都从写库读
		if(!DataSourceType.write.getType().equals(DataSourceContextHolder.getReadOrWrite())){
	...
	@Before("execution(* com.fei.springboot.memory9991.service..*.insert*(..)) or execution(* com.fei.springboot.memory9991.service..*.update*(..)) or execution(* com.fei.springboot.memory9991.service..*.add*(..)) or execution(* com.fei.springboot.memory9991.service..*.delete*(..))"
